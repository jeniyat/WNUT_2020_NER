...
def main(argv=None,  input_folder_main= "checked_annotations_training/", output_folder = 'Conlll_Output_ANN/' ):
    if argv is None:
        argv = sys.argv
    ...
    for dir,_,_ in sorted(os.walk(input_folder_main)):
        try:
            phase_num = int(dir.replace(input_folder_main, "").replace("phase_",""))
            list_of_phases.append(phase_num)
        except: 
            continue
    # print(list_of_phases)   
    for i in list_of_phases:
        phase_name="phase_"+str(i).zfill(2) +"/"
